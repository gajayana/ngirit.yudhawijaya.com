...
export async function checkUserRole(
  ...
  if (error || !data) {
    throw createError({
      statusCode: 403,
      statusMessage: 'Forbidden - No user data found',
    });
  }

  if ((data as UserData).is_blocked) {
    throw createError({
      statusCode: 403,
  ...
  if (!allowedRoles.includes((data as UserData).role)) {
    throw createError({
      statusCode: 403,
  ...
  return { userId, role: (data as UserData).role };
}
